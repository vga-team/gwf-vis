...
def _create_value_table(
    db_connection: sqlite3.Connection, dimensions: Sequence[Dimension]
):
    ...
    sql = f"""
        CREATE TABLE value (
            ...
            variable INTEGER NOT NULL,
            {f' INTEGER, {_NEW_LINE_CHARACTER}'.join(dimension_column_names)}{', ' if dimension_column_names is not None else ''}
            value FLOAT,
            FOREIGN KEY (variable) REFERENCES variable (id),
...
def _fill_location_table(
    db_connection: sqlite3.Connection, locations: Sequence[Location]
):
    sql = f"""
    ...
    for location in locations:
        _execute_sql(
            ...
            [
                location.id,
                json.dumps(location.geometry, indent=2),
                json.dumps(location.metadata, indent=2),
            ],
        )


def _fill_dimension_table(
    db_connection: sqlite3.Connection, dimensions: Sequence[Dimension]
):
    sql = f"""
    ...
    for dimension in dimensions:
        _execute_sql(
            ...
            [
                ...
                json.dumps(dimension.value_labels),
            ],
        )


def _fill_variable_table(
    db_connection: sqlite3.Connection, variables: Sequence[Variable]
):
    sql = f"""
...
def _fill_variable_dimension_table(
    db_connection: sqlite3.Connection, variables: Sequence[Variable]
):
    sql = f"""
    ...
    for value in values:
        ...
        sql = f"""
            INSERT INTO value (
                location, 
                variable, 
                {f', {_NEW_LINE_CHARACTER}'.join(dimension_column_names)}{', ' if dimension_column_names is not None else ''}
                value
...
def _query_db_table(
    db_connection: sqlite3.Connection, table_name: str, columns: Sequence[str] = None
):
    db_cursor = db_connection.cursor()
...
def read_gwfvis_db(path: str):
    ...
    query_result = _query_db_table(db_connection=db_connection, table_name="info")
    info = list(
        map(
            lambda d: Info(key=d["key"], value=d["value"], label=d["label"]),
            query_result,
        )
    )

    query_result = _query_db_table(db_connection=db_connection, table_name="location")
    locations = list(
        map(
            lambda d: Location(
                id=d["id"],
    ...
    query_result = _query_db_table(db_connection=db_connection, table_name="dimension")
    dimensions = list(
        map(
            lambda d: Dimension(
                id=d["id"],
                ...
                value_labels=json.loads(d["value_labels"]),
            ),
            query_result,
        )
    )

    query_result = _query_db_table(db_connection=db_connection, table_name="variable")
    variables = list(
        map(
            lambda d: Variable(
                id=d["id"],
    ...
    query_result = _query_db_table(db_connection=db_connection, table_name="value")
    values = list(
        map(
            lambda value: Value(
                location=next(
                    filter(
                        lambda location: location.id == value["location"], locations
                    ),
                    None,
                ),
                variable=next(
                    filter(
                        lambda variable: variable.id == value["variable"], variables
                    ),
                    None,
                ),
                value=value["value"],
                    ...
                    map(
                        lambda key: (
                            next(
                                dimension
                                for dimension in dimensions
                                if dimension.id == int(key[10:])
                            ),
                            value[key],
                        ),
                        list(
                            filter(
                                lambda key: key.startswith("dimension_"), value.keys()
                            )
                        ),
                    )
                ),
